...
export const buildTheme = (mainColor: string) => {
  ...
  const secondaryContainer = isDark
    ? base.clone().lighten(tooLight.includes(mainColor) ? 25 : 50).toHexString()
    : base.clone().lighten(tooDark.includes(mainColor) ? 25 : 15).toHexString();

  const elevationLevel2 = isDark
    ? base.clone().lighten(tooLight.includes(mainColor) ? 30 : 45).toHexString()
    : base.clone().lighten(tooDark.includes(mainColor) ? 30 : 20).toHexString();
